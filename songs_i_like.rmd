...
my_features$mode_name <- as.factor(my_features$mode)
levels(my_features$mode_name) <- mode_names
...
Other people rank songs in the key of F most popular among the songs I like, but key is not a great predictor of popularity.
```{r keys others}
my_features %>% 
   ...
   mutate(key_color = ifelse(str_detect(pitch,"#"),"black","white")) %>% 
   ggplot(aes(pitch,avg_popularity,fill = key_color)) + geom_col() +
...
my_features %>% 
   mutate(key_color = ifelse(str_detect(pitch,"#"),"black","white")) %>% 
   group_by(pitch) %>% 
   ggplot(aes(pitch,fill=key_color)) + geom_bar() + 
   scale_fill_manual(values = c("black","white")) +
