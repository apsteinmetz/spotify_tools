...
rm(playlists)
```
Get audio features for top artists
```{r}
...
spot_artist_names <- artist_features_raw %>% select(artist_name) %>% unique()
SBB_artist_names <- SBB_all_tracks %>% transmute(artist_name)

full_join(SBB_artist_names,spot_artist_names)

SBB_all_tracks <- playlists %>% filter(DJ == "CF") %>% 
   transmute(artist_name = Artist,track_name = Title) %>% 
   unique() 
